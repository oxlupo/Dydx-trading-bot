    ...
    for index, row in df.iterrows():
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) >= ZSCORE_THRESH:
                ...
                # PLace trade
                if is_base_open and not is_quote_open:

                    # Determine side
                    base_side = "BUY" if z_score < 0 else "SELL"
                    quote_side = "BUY" if z_score > 0 else "SELL"

                    # Get acceptable in string format with correct number of decimals
                    base_price = series_1[-1]
                    quote_price = series_2[-1]
                    accept_base_price = float(base_price) * 1.01 if z_score < 0 else float(base_price) * 0.99
                    accept_quote_price = float(base_price) * 1.01 if z_score > 0 else float(base_price) * 0.99
                    fail_base_price = float(base_price) * 0.05 if z_score < 0 else float(base_price) * 1.7
                    ...
                    accept_failsafe_base_price = format_number(fail_base_price, base_tick_size)
                    ...
                    quote_size = format_number(base_quantity, base_step_size)

                    # Ensure size
                    base_min_order_size = markets["markets"][base_market]["minOrderSize"]
                    quote_min_order_size = markets["markets"][quote_size]["minOrderSize"]
